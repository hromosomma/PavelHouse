...
def first_level():
    ...
    hero_flag = True #какой спрайт будет нарисован
    coins_time_span = 3
    coins_for_one_time = 1 # сколько монеток отрисуется за раз
    pygame.time.set_timer(COINS_TIMER, coins_time_span * 1000 // 1)# время до появления монетки
    counter = 0 # счетчик для снижения скорости появлениия монетки через каждые три операции
    coins_flag = False
            ...
            if event.type == COINS_TIMER:
                ...
                if counter % 3 == 0:
                    ...
                    if coins_time_span == 0:
                        coins_flag = True
                pygame.time.set_timer(COINS_TIMER, coins_time_span * 1000 // 1)
